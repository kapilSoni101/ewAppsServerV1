...
namespace ewApps.Shipment.DS {
    ...
    public interface IRoleLinkingDS:IBaseDS<RoleLinking> {
        ...
        /// <returns></returns>
        Task AddShipmentRolelinkingAsync(TenantUserAppManagmentDTO roleLinkingDTO, CancellationToken cancellationToken = default(CancellationToken));

        Task<RoleUpdateResponseDTO> UpdateShipmentRoleAsync(TenantUserAppManagmentDTO tenantUserAppManagmentDTO, CancellationToken cancellationToken = default(CancellationToken));
      }
}
