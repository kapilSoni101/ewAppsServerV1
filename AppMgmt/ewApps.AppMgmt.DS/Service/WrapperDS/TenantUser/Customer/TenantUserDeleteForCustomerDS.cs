...
namespace ewApps.AppMgmt.DS {
    public class TenantUserDeleteForCustomerDS:ITenantUserDeleteForCustomerDS {
        ...
        public TenantUserDeleteForCustomerDS(ITenantUserDS tenantUserDS, IIdentityServerDS identityServerDS,
        ...
        public async Task<DeleteUserResponseDTO> DeleteTenantUserAsync(TenantUserIdentificationDTO tenantUserIdentificationDTO) {
            ...
            await DeleteUserHierarchy(tenantUserIdentificationDTO.TenantUserId, tenantUserIdentificationDTO.TenantId, tenantUserIdentificationDTO.BusinessPartnerTenantId);
            ...
            //3) tokeninfo
            //4) rolelinking
            await _tenantUserDS.DeleteUserDependencyAsync(tenantUserIdentificationDTO.TenantUserId, tenantUserIdentificationDTO.TenantId, tenantUserIdentificationDTO.AppId, tenantUserIdentificationDTO.BusinessPartnerTenantId);
        ...
        private async Task DeleteUserHierarchy(Guid tenantUserId, Guid tenantId, Guid? businessPartnerTenantId) {
            ...
            if(tenantUser != null && tenantUser.Deleted == false) {
                ...
                List<TenantUserAppLinking> tenantUserAppLinkingList = (await _tenantUserAppLinkingDataService.FindAllAsync(u => u.TenantId == tenantId && u.TenantUserId == tenantUserId && u.BusinessPartnerTenantId == businessPartnerTenantId && u.Deleted == false)).ToList();
        ...
        private async Task ChangePrimaryUserWhenDeleted(UserTenantLinking userTenantLinking) {
            List<UserTenantLinking> userTenantLinkingList = (await _userTenantLinkingDS.FindAllAsync(ut => ut.TenantId == userTenantLinking.TenantId && ut.BusinessPartnerTenantId == userTenantLinking.BusinessPartnerTenantId && ut.UserType == (int)UserTypeEnum.Customer && ut.Deleted == false)).ToList();
