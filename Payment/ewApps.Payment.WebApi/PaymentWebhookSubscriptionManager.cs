        ...
        public async void InitWebhookSubscription(IApplicationBuilder app) {
            try {
                using(IServiceScope scope = app.ApplicationServices.CreateScope()) {
                    WebhookSubscriptionManager manager = scope.ServiceProvider.GetRequiredService<WebhookSubscriptionManager>();
                ...
                using(var serviceScope = _serviceProvider.CreateScope()) {
                    ...
                    foreach(WebhookEvent item in webhookEvent) {
                        ...
                        if(item.EventName == "VeriCheckPayment.StatusChange") {
                            ...
                            _loggerService.Log(LogLevel.Information, "START 2: Payment(HandleEvent) Condition matched 2nd ");
                        }
                        else {
