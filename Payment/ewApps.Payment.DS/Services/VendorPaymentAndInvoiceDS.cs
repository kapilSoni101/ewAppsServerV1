...
namespace ewApps.Payment.DS {
    ...
    /// </summary>
    public class VendorPaymentAndInvoiceDS:IVendorPaymentAndInvoiceDS {

        #region Local variables

        IPaymentDataService _paymentDS;
        IQPaymentDS _qPaymentDS;
        IQAPInvoiceDS _qInvoiceDS;
        ...
        ILogger<VendorPaymentAndInvoiceDS> _logger;
        ...
        /// </summary>
        public VendorPaymentAndInvoiceDS(ILogger<VendorPaymentAndInvoiceDS> logger, IPaymentDataService paymentDS,
               ...
               IQPaymentDS qPaymentDS, IQAPInvoiceDS qInvoiceDS, IQInvoiceItemDS qInvoiceItemDS,
               ICreditCardAccountDS creditCardAccountDS, IOptions<PaymentAppSettings> appSetting,
               IPaymentNotificationHandler paymentNotificationHandler, IPaymentNotificationRecipientDS paymentNotificationDS) {
            _logger = logger;
        ...
        /// <returns></returns>
        public async Task<BAAPInvoiceViewDTO> GetInvoiceByInvoiceIdAsync(Guid invoiceId, CancellationToken token = default(CancellationToken)) {
            BAAPInvoiceViewDTO model = await _qInvoiceDS.GetInvoiceDetailByInvoiceIdAsync(invoiceId, token);
            if(model != null) {
        ...
        private async Task<BAAPInvoiceEntityDTO> GetInvoiceEntityAsync(Guid invoiceId, CancellationToken token = default(CancellationToken)) {
        ...
        /// <param name="token"></param>
        /// <returns></returns>
        private async Task<UserTenantLinkingEntityDTO> GetPrimaryUserOfVendorAsync(Guid BusinessPartnerTenantId, CancellationToken token = default(CancellationToken)) {
            try {
                return await _qInvoiceDS.GetPrimaryUserOfVendorAsync(BusinessPartnerTenantId, token);
            }
            catch {
                return null;
            }
        ...
        /// <returns></returns>
        public async Task<List<BAAPInvoiceDTO>> GetInvoiceByTenantAsync(ListDateFilterDTO filter, CancellationToken token = default(CancellationToken)) {
        ...
        public async Task<List<BAAPInvoiceDTO>> GetBAAPInvoiceDTOListByInvoiceIdAsync(List<Guid> invoiceId, CancellationToken token = default(CancellationToken)) {
            return await _qInvoiceDS.GetBAAPInvoiceDTOListByInvoiceIdAsync(invoiceId, token);
        }
        ...
        /// <param name="token">A token that can be use to propogate async operation cancel notification. Using this token async operation duration can be control.</param>
        /// <returns></returns>
        public async Task<List<BAAPInvoiceDTO>> GetCancelInvoiceByTenantAsync(ListDateFilterDTO filter, CancellationToken token = default(CancellationToken)) {
            Guid tenantId = _sessionManager.GetSession().TenantId;
        ...
        public async Task<List<BAAPInvoiceDTO>> GetInvoiceByVendorAsync(ListDateFilterDTO filter, CancellationToken token = default(CancellationToken)) {
            return await _qInvoiceDS.GetInvoiceByVendorAsync(filter, token);
        }
        ...
        /// <param name="token">A token that can be use to propogate async operation cancel notification. Using this token async operation duration can be control.</param>
        /// <returns></returns>
        public async Task<List<BAAPInvoiceDTO>> GetQuickPayInvoicesByTenantAsync(ListDateFilterDTO filter, CancellationToken token = default(CancellationToken)) {
            Guid tenantId = _sessionManager.GetSession().TenantId;
        ...
        public async Task<List<BAAPInvoiceDTO>> GetVendorPaybleinvoicesByVendorIdAsync(Guid vendorId, CancellationToken token = default(CancellationToken)) {
            return await _qInvoiceDS.GetVendorPaybleinvoicesByVendorIdAsync(vendorId, token);
        }
        ...
        public async Task<IList<VendorPaymentDetailDQ>> GetAPInvoiceFilterTenantPaymentHistoryAsync(ListDateFilterDTO filter, CancellationToken token = default(CancellationToken)) {
            IList<VendorPaymentDetailDQ> list = await _qPaymentDS.GetFilterVendorTenantPaymentHistoryAsync(filter, token);
            return list;
        }
        ...
        /// <param name="token"></param>
        /// <returns>return paid invoice payment history.</returns>
        public async Task<IList<PaymentDetailDQ>> GetVendorFilterPaymentHistoryAsync(ListDateFilterDTO filter, CancellationToken token = default(CancellationToken)) {
        ...
        /// <param name="token"></param>
        /// <returns>return paid invoice payment history.</returns>
        public async Task<IList<PaymentDetailDQ>> GetAPInvoiceVoidFilterTenantPaymentHistoryAsync(ListDateFilterDTO filter, CancellationToken token = default(CancellationToken)) {
            // Get the items in sorted order by created date and Payment Id.
        ...
        /// <param name="token"></param>
        /// <returns>return paid invoice payment history.</returns>
        public async Task<IList<PaymentDetailDQ>> GetAPInvoiceSattledFilterTenantPaymentHistoryAsync(ListDateFilterDTO filter, CancellationToken token = default(CancellationToken)) {
            // Get the items in sorted order by created date and Payment Id.
        ...
        /// <param name="token"></param>
        /// <returns></returns>   
        public async Task<IList<VendorPaymentDetailDQ>> GetVendorInvoicePaymentHistoryByInvoiceAsync(Guid invoiceId, CancellationToken token = default(CancellationToken)) {
            // Get the items in sorted order by created date and Payment Id.
            return await _qPaymentDS.GetVendorPaymentHistoryByInvoiceAsync(invoiceId, token);
        }
        ...
        public async Task<PaymentDetailDQ> AddPaymentsAsync(AddPaymentDTO[] e, CancellationToken token = default(CancellationToken)) {
            ...
            BAAPInvoiceEntityDTO invoice;
            int paymentCount = e.Length;
            AddPaymentDTO addPaymentDto = null;
            List<BAAPInvoiceEntityDTO> listInvoice = new List<BAAPInvoiceEntityDTO>();
            ...
            if(paymentCount > 0) {
                ...
                List<BAAPInvoiceEntityDTO> updateInvoiceList = new List<BAAPInvoiceEntityDTO>();
                ...
                for(int i = 0; i < paymentCount; i++) {
                    ...
                    if(session != null) {
                        BAAPInvoiceEntityDTO dto = this.GenerateUpdateInvoiceEntity(e[i], session.TenantUserId, invoice);
                        updateInvoiceList.Add(dto);
                    }
                    else {
                        Guid updateBy = paymentEntity.UpdatedBy != Guid.Empty ? paymentEntity.UpdatedBy : invoice.UpdatedBy;
                        BAAPInvoiceEntityDTO dto = this.GenerateUpdateInvoiceEntity(e[i], updateBy, invoice);
                        updateInvoiceList.Add(dto);
                    }
                }
                ...
                */
                ...
                try {
                    ...
                    if(iscredicardPay && tInfo != null && tInfo.IntegratedMode) {
                        // Notify SAP for invoice payment.
                        await PushPaymentToSAPAsync(addPaymentDto, e, listInvoice, paymentEntity, token);
                    }
        ...
        private async Task<PaymentDQ> AddPaymentAsync(AddPaymentDTO e, PaymentDQ paymentDTO, PreAuthPayment preAuthPayment, Guid businessPartnerTenantId, CancellationToken token = default(CancellationToken)) {
            ...
            if(_sessionManager.GetSession() != null) {
                ...
                UserTenantLinkingEntityDTO link = await GetPrimaryUserOfVendorAsync(businessPartnerTenantId, token);
                if(link != null) {
        ...
        /// <param name="e" entity to be added</param>
        /// <returns>Added entity</returns>
        private async Task<PaymentDQ> AddPaymentInvoiceLinkingAsync(AddPaymentDTO e, PaymentDQ paymentDTO, BAAPInvoiceEntityDTO invoice, CancellationToken token = default(CancellationToken)) {
            //Maps Payment entity from DTO  
        ...
        private BAAPInvoiceEntityDTO GenerateUpdateInvoiceEntity(AddPaymentDTO e, Guid updatedBy, BAAPInvoiceEntityDTO invoice) {
        ...
        private void ValidateInvoicePaidAmount(BAAPInvoiceEntityDTO invoice, decimal amountPaying, bool isBusinessCurrency) {
            if(invoice != null) {
        ...
        /// <param name="e" entity to be added</param>
        /// <returns>Added entity</returns>
        private async Task<bool> UpdateInvoiceAfterPaymentAsync(List<BAAPInvoiceEntityDTO> listInv, CancellationToken token = default(CancellationToken)) {
        ...
        /// <param name="e" entity to be added</param>
        /// <returns>Added entity</returns>
        private async Task<object> PushPaymentToSAPAsync(AddPaymentDTO e, AddPaymentDTO[] listPayModel, List<BAAPInvoiceEntityDTO> listInvoice, Entity.Payment pay, CancellationToken token = default(CancellationToken)) {
            RequestOptions requestOptions = new RequestOptions();
            requestOptions.AcceptType = AcceptMediaTypeEnum.JSON;
            requestOptions.Method = "payment";
            requestOptions.MethodData = GenerateSAPPushPayModel(e, listPayModel, listInvoice, pay);
        ...
        private SAPPaymentPushRequestDTO GenerateSAPPushPayModel(AddPaymentDTO addPaymentDto, AddPaymentDTO[] listPayModel, List<BAAPInvoiceEntityDTO> listInvoice, Entity.Payment pay) {
            SAPPaymentPushRequestDTO sapModel = new SAPPaymentPushRequestDTO();
            sapModel.BAPaymentTransId = pay.TransactionId;
            sapModel.Notes = addPaymentDto.Note;
            sapModel.PaymentDate = pay.CreatedOn.Value;
            sapModel.ARInvoices = new List<BAPaymentInvoiceSyncDTO>();
            string erpCustomerKey = "";
            for(int i = 0; i < listPayModel.Length; i++) {
                BAAPInvoiceEntityDTO invEntDTO = listInvoice.Find(inv => inv.ID == listPayModel[i].InvoiceId);
                if(invEntDTO == null)
                    continue;
                BAPaymentInvoiceSyncDTO payInv = new BAPaymentInvoiceSyncDTO();
                payInv.ERPARInvoiceId = invEntDTO.ERPAPInvoiceKey;
                erpCustomerKey = invEntDTO.ERPVendorKey;
