...
<!--Subject– <Publisher Co. Name> Customer Portal: Transaction <transaction ID> is Void Requested.
Dear <Customer Co. Name>, 

Void is initiated by <Business Co. Name> for a transaction with below details on <date & time>: 
    ...
    <html>
      <head>
        <title>
          ...
          <xsl:text> </xsl:text>
          <xsl:text> Customer Portal: Transaction </xsl:text>
          <xsl:text> </xsl:text>
          <xsl:value-of select="$transactionID"/>
          <xsl:text> </xsl:text>
          <xsl:text> is Void Requested. </xsl:text>
        </title>
        <style type="text/css">
           ...
           opacity:0;height:0px;
            ...
            <div>
             ...
             position: absolute;">
                  ...
                  <xsl:text> Void is initiated by </xsl:text>
                  <xsl:text> </xsl:text>
                  <xsl:value-of select="$businessCompanyName"/>
                  <xsl:text> </xsl:text>
                  <xsl:text> for a transaction with below details on </xsl:text>
                  <xsl:text> </xsl:text>
