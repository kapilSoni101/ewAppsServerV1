...
namespace ewApps.AppPortal.QData {
    public class QVendorAppRepository:QBaseRepository<QAppPortalDbContext>, IQVendorAppRepository {

        public QVendorAppRepository(QAppPortalDbContext context) : base(context) {
        }

        ///<inheritdoc/>
        public async Task<List<AppInfoDTO>> GetAllVendorApplicationsAsync(Guid businessPartnerTenantId, string defaultFreeAppKey) {
            string query = @"select AP.ID , ap.IdentityNumber, PAS.Name, PAS.ThemeId, PAS.Active, ap.AppKey 
                            ...
                            union 
                            select AP.ID , ap.IdentityNumber, AP.Name, AP.ThemeId, AP.Active, ap.AppKey 
                            from am.App AP
                            where AppKey =@appkey";
