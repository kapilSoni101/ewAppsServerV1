...
<!--Subject – <Platform Co. Name> Platform Portal: You have an invitation from <Platform Co. Name>.

Dear <Invited User Name>,

<Platform Inviting User Name> hereby invites you to join <Platform Co. Name> Platform Portal. It seems that this email id <email id> already exists on <portal name on which set password done>, please use the existing credentials for login.
...
Portal URL: <Platform Portal URL>
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="platformInvitingUser"/>  
    ...
    <html>
      <head>
        <title>
          ...
          <xsl:text> </xsl:text>
          <xsl:text>Platform Portal: You have an invitation from </xsl:text>
          <xsl:text> </xsl:text>
          <xsl:value-of select="$platformCompanyName"/>
          <xsl:text>. </xsl:text>
        </title>
           ...
           opacity:0;height:0px;
            ...
            <div>
             ...
             position: absolute;">
                  ...
                  <xsl:text>, </xsl:text>
                  <xsl:value-of select="$platformInvitingUser"/>
                  <xsl:text> </xsl:text>
                  <xsl:text> hereby invites you to join </xsl:text>
                  <xsl:text> </xsl:text>
                  <xsl:value-of select="$platformCompanyName"/>
                  <xsl:text> </xsl:text>
                  <xsl:text>  Platform Portal. It seems that this email id</xsl:text>
                  <xsl:text> </xsl:text>
                  ...
                  <xsl:value-of select="$platformPortalURL"/>-->
            ...
            <td valign="top" align="center" class="textContent">
              <div style="text-align:left;font-family:'Lucida Sans Unicode','Lucida Grande', sans-serif;font-size:14px;margin-bottom:0;color:#000;line-height:19px;">
                ...
                <p style="font-family:'Lucida Sans Unicode','Lucida Grande', sans-serif;font-size:14px; color:#000;">
                  ...
                  <xsl:text> </xsl:text>
                  <b>
                    <xsl:text> Platform Portal.</xsl:text>
                  </b>
                  <xsl:text> </xsl:text>
