...
<!--
Subject – <Publisher Co. Name> Business Portal: Your user account on <Publisher Co. Name> Business Portal – Setup is <new status> now.  

Dear <Business User Name>,  
 
Your user account status on <Publisher Co. Name> Business Portal - Setup has been changed from <Old Status> to <new status>. Please contact your Portal Admin for more details.
    ...
    <html>
      <head>
        <title>
          ...
          <xsl:text> </xsl:text>
          <xsl:text> Business Portal: Your user account on  </xsl:text>
          <xsl:text> </xsl:text>
          <xsl:value-of select="$publisherCompanyName"/>
          <xsl:text> </xsl:text>
          <xsl:text>Business Portal – Setup is  </xsl:text>
          <xsl:text> </xsl:text>
           ...
           opacity:0;height:0px;
            ...
            <div>
             ...
             position: absolute;">
                  ...
                  <xsl:value-of select="$publisherCompanyName"/>
                  <xsl:text> </xsl:text>
                  <xsl:text> Business Portal - Setup has been changed from </xsl:text>
                  <xsl:text> </xsl:text>
                  <xsl:value-of select="$oldStatus"/>
                  <xsl:text> to </xsl:text>
                  <xsl:value-of select="$newStatus"/>
                  <xsl:text> </xsl:text>
                  <xsl:text> . Please contact your Portal Admin for more details. </xsl:text>
                  <xsl:text> </xsl:text>
            ...
            <td valign="top" align="center" class="textContent">
              <div style="text-align:left;font-family:'Lucida Sans Unicode','Lucida Grande', sans-serif;font-size:14px;margin-bottom:0;color:#000;line-height:19px;">
                ...
                <p style="font-family:'Lucida Sans Unicode','Lucida Grande', sans-serif;font-size:14px; color:#000;">
                  ...
                  <xsl:text> </xsl:text>
                  <b>
                    <xsl:text> Business Portal - Setup </xsl:text>
                  </b>
                  <xsl:text> </xsl:text>
                  <xsl:text> has been changed from </xsl:text>
                  <xsl:text> </xsl:text>
                  <b>
                    <xsl:value-of select="$oldStatus"/>
                  </b>
