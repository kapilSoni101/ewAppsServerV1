...
<!--Subject – <Publisher Co.="" Name=""> Business Portal: You have subscribed to a new subscription <Plan Name=""> for <Application Name="">.

Dear <Business Co.="" Name="">
, 
You have subscribed <Application Name=""> application on <Publisher Co.="" Name=""> Business Portal with below subscription plan details.

Subscription Plan Name:
Plan Start Date: 
Plan Expiry Date / Auto Renewal Date:
Auto Renewal: 
...
You received this email because you are subscribed to Portal Alerts from <Business Co.="" 
Name="">.-->
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="planStartDate"/>
  <xsl:param name="planExpiryDate"/>
  <xsl:param name="autoRenewal"/>
    ...
    <html>
      <head>
        <title>
          ...
          <xsl:text> </xsl:text>
          <xsl:text> Business Portal: You have subscribed to a new subscription </xsl:text>
          <xsl:text> </xsl:text>
           ...
           opacity:0;height:0px;
            ...
            <div>
             ...
             position: absolute;">
                  ...
                  <xsl:text>, </xsl:text>
                  <xsl:text> You have subscribed </xsl:text>
                  <xsl:text> </xsl:text>
                  <xsl:value-of select="$applicationName"/>
                  <xsl:text> </xsl:text>
                  ...
                  <xsl:value-of select="$publisherCompanyName"/>
                  <xsl:text> </xsl:text>
                  <xsl:text> Business Portal with below subscription plan details. </xsl:text>
                  <xsl:text> </xsl:text>
                  <xsl:text> Subscription Plan Name:</xsl:text>
                  <xsl:text> </xsl:text>
                  <xsl:value-of select="$planName"/>
                  <xsl:text> </xsl:text>
                  <xsl:text> Plan Start Date:</xsl:text>
                  <xsl:text> </xsl:text>
                  <xsl:value-of select="$planStartDate"/>
                  <xsl:text> </xsl:text>
                  <xsl:text> Plan Expiry Date / Auto Renewal Date:</xsl:text>
                  <xsl:text> </xsl:text>
                  <xsl:value-of select="$planExpiryDate"/>
                  <xsl:text> </xsl:text>
                  <xsl:text> Auto Renewal:</xsl:text>
                  <xsl:text> </xsl:text>
                  <xsl:value-of select="$autoRenewal"/>
                  <xsl:text> </xsl:text>
