...
<!--Subject – <Publisher Co. Name> Business Portal: Company account is deleted.

Dear <Business Co. Name>,

<Publisher Co. Name> has deleted <Business Co. Name> company account on <Publisher Co. Name> Business Portal. You can contact <Platform Co. Name> for more details.
    ...
    <html>
      <head>
        <title>
          ...
          <xsl:text> </xsl:text>
          <xsl:text>  Business Portal: Company account is deleted.</xsl:text>
        </title>
        <style type="text/css">
           ...
           opacity:0;height:0px;
            ...
            <div>
             ...
             position: absolute;">
                  ...
                  <xsl:text> </xsl:text>
                  <xsl:text>  has deleted</xsl:text>
                  <xsl:text> </xsl:text>
                  <xsl:value-of select="$businessCompanyName"/>
                  <xsl:text> company account on </xsl:text>
                  <xsl:text> </xsl:text>
                  <xsl:value-of select="$publisherCompanyName"/>
                  <xsl:text> Business Portal. You can contact</xsl:text>
                  <xsl:text> </xsl:text>
                  <xsl:value-of select="$platformCompanyName"/>
                  <xsl:text> </xsl:text>
            ...
            <td valign="top" align="center" class="textContent">
              <div style="text-align:left;font-family:'Lucida Sans Unicode','Lucida Grande', sans-serif;font-size:14px;margin-bottom:0;color:#000;line-height:19px;">
                ...
                <p style="font-family:'Lucida Sans Unicode','Lucida Grande', sans-serif;font-size:14px; color:#000;">
                  ...
                  <xsl:text>  You can contact</xsl:text>
                  <xsl:text> </xsl:text>
                  <b>
                    <xsl:value-of select="$platformCompanyName"/>
                  </b>
