    ...
    <html>
      <head>
        <title>
          ...
          <xsl:text> </xsl:text>
          <xsl:text> Publisher Portal: Publisher User </xsl:text>
          <xsl:text> </xsl:text>
          <xsl:value-of select="$publisherUserName"/>
          <xsl:text> </xsl:text>
          <xsl:text> has been permanently deleted.</xsl:text>
        </title>
        <style type="text/css">
           ...
           opacity:0;height:0px;
            ...
            <div>
             ...
             position: absolute;">
                  ...
                  <xsl:value-of select="$publisherName"/>
                  <xsl:text>, </xsl:text>
                  <xsl:text> Publisher User </xsl:text>
                  <xsl:text> </xsl:text>
                  <xsl:value-of select="$publisherUserName"/>
                  <xsl:text> has been permanently deleted from </xsl:text>
                  <xsl:text> </xsl:text>
                  <xsl:value-of select="$publisherName"/>
                  <xsl:text> </xsl:text>
                  <xsl:text> Publisher Portal by </xsl:text>
                  <xsl:text> </xsl:text>
                  <xsl:value-of select="$deletedBy"/>
                  <xsl:text> </xsl:text>
                  <xsl:text> on </xsl:text>
                  <xsl:text> </xsl:text>
                  <xsl:value-of select="$dateTime"/>
                  <xsl:text>. This user won’t be able to access the portal. </xsl:text>
